    ...
    private fun getResult() {
        ...
        try {
            ...
            if (decompressionStops.isEmpty()) {
                ...
                var message = ""
                for ((step, stopInfo) in decompressionStops.withIndex()) {
                    val depthStop = (step + 1) * 3
                    message += "[${step + 1}] Всплывайте на $depthStop м с выдержкой: $stopInfo\n\n"
                }
                decompResult.text = "Рекомендуем:\n\n$message"
            }
    ...
    private fun table(nD: Int, nT: Int): List<String> {
        ...
        for (line in lines) {
            ...
            if (columns[0].toIntOrNull() == nD && columns[1].toIntOrNull() == nT) {
                return columns.drop(3).filter { it.isNotEmpty() }
            }
        }
        return emptyList()
    }
